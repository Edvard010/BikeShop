...
namespace BikeShop.Services
{
    public class ClientService
    {
        ...
        public (bool, string) DeleteFromBasket(long id, ItemDto bikeItem) //dodać sprawdzanie czy taki rower jest w koszyku, ale może inaczej niż wyżej
        {
            var client = _context.Clients.SingleOrDefault(x => x.Id == id);
            if (client == null)
            {
                return (false, "opis błędu");
            }
            var id2 = bikeItem.Id;
            var bike = client.Basket.SingleOrDefault(x => x.Id == id2);
            if (bike == null)
            {
                return (false, "opis błędu2");
            }
